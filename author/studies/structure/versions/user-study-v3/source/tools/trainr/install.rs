#!/bin/bash
# install.sh – TRAINR installer with binary compilation via SHC
...
# 3. Compile trainr to native binary using shc
echo "Compiling $TARGET_NAME into binary with shc..."
shc -f "$ORIGINAL_SCRIPT"

# Output: trainr.x (binary) and trainr.x.c (source)
...
echo "trainr install complete. Run 'trainr' anywhere to begin."
