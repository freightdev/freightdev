...
#! ╚═══════════════════════════════╝
#? https://github.com/freightdev/main/plugins/zsh-zenvctr

set -euo pipefail

# Defaults (override by loading custom env if needed)
: "${ENV_DIR:=$HOME/.zshenv.d}"
: "${ENV_SRC:=$HOME/.zshsrc.d}"
: "${ENV_SRC_BACKUP:=$ENV_SRC/backups}"
: "${EDITOR:=nano}"
...
case "$1" in
    ...
    rename)
        ...
        OLD=$(normalize "$2")
        NEW=$(normalize "$3")
        OLD_FILE="$ENV_SRC/$OLD"
        NEW_FILE="$ENV_SRC/$NEW"
    ...
    lru)
        ...
        if [[ -d "$ENV_SRC" ]]; then
            find "$ENV_SRC" -type f -name '*.zsh' -exec stat -c "%X %n" {} + 2>/dev/null |
                sort -nr | head -10 | while read -r timestamp file; do
                    echo "$(date -d @$timestamp '+%Y-%m-%d %H:%M:%S') $file"
                done
        else
            echo "Directory $ENV_SRC not found."
        fi
        ;;
    ...
    audit)
        ...
        if [[ -d "$ENV_SRC" ]]; then
            find "$ENV_SRC" -type f -name '*.zsh' | while read -r file; do
                if [[ -r "$file" ]]; then
    ...
    list)
        local category="$2"
        if [[ -z "$category" ]]; then
            echo "Listing all $ENV_SRC scripts:"
            if [[ -d "$ENV_SRC" ]]; then
                find "$ENV_SRC" -type f -name '*.zsh' | sort
            else
                echo "Directory $ENV_SRC not found."
            fi
        else
            local target_dir="$ENV_SRC/$category"
            if [[ -d "$target_dir" ]]; then
                ...
                if [[ -d "$ENV_SRC" ]]; then
                    find "$ENV_SRC" -type d -mindepth 1 -maxdepth 1 | while read -r dir; do
                        echo "  - $(basename "$dir")"
                    done
                fi
            fi
    ...
    backup)
        ...
        if [[ -d "$ENV_SRC" ]]; then
            ...
            echo "Error: Source directory $ENV_SRC not found."
            exit 1
        fi
        ;;
    ...
    search)
        [[ -z "$2" ]] && usage
        echo "Searching for '$2' in $ENV_SRC..."
        if [[ -d "$ENV_SRC" ]]; then
            if command -v rg >/dev/null 2>&1; then
                rg --color=auto -Hn "$2" "$ENV_SRC"
            elif command -v grep >/dev/null 2>&1; then
                grep -rn --color=auto "$2" "$ENV_SRC"
            else
