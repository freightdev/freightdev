...
Make sure GPG keys, SSH keys, git access, and API tokens are pre-configured.
...
main() {
  ...
  case "${1:-help}" in
    index) update_index ;;
    search) shift; [[ $# -ge 1 ]] || die "search requires a regex pattern"; search_index "$1" ;;
    create) shift; [[ $# -ge 1 ]] || die "create requires source directory"; create_backup "$1" "${2:-}" ;;
    restore) shift; [[ $# -ge 1 ]] || die "restore requires backup name"; restore_backup "$1" "${2:-}" ;;
    push) shift; push_backup "${1:-}" ;;
    pull) shift; [[ $# -ge 1 ]] || die "pull requires backup name"; pull_backup "$1" "${2:-}" ;;
    view) create_view_symlinks ;;
    cleanup) cleanup_backups ;;
    notify) shift; [[ $# -ge 2 ]] || die "notify requires subject and body"; send_email_notification "$1" "$2" ;;
    help|*) show_help ;;
  esac
}
